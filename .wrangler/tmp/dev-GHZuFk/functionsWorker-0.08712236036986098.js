...
// .wrangler/tmp/pages-7EJ4iP/functionsWorker-0.08712236036986098.mjs
...
__name2(corsFor, "corsFor");
async function hmacSession(sessionId, secret) {
...
__name(hmacSession, "hmacSession");
__name2(hmacSession, "hmacSession");
async function sampleGhost(env, game, band, outcome) {
  return { hash: "gh_" + band + outcome, snapshot: { v: 1, game_v: "dev", seed: 0, actions: [] } };
}
__name(sampleGhost, "sampleGhost");
__name2(sampleGhost, "sampleGhost");
async function saveSnapshotAndMeta(env, req, snapshot, sessionId) {
  return { playerHash: "phash_dev" };
}
__name(saveSnapshotAndMeta, "saveSnapshotAndMeta");
__name2(saveSnapshotAndMeta, "saveSnapshotAndMeta");
async function saveReplay(env, game, playerHash, ghostHash, replay, result) {
  return "rpl_dev";
}
__name(saveReplay, "saveReplay");
__name2(saveReplay, "saveReplay");
async function runServerSim(req, ghost) {
  const replay = { v: 1, game_v: req.client_build, initial: { player: req.player_setup.board, ghost: [0, 0, 0] }, events: [{ t: 0, e: "spawn", side: "P", slot: 0, unit: req.player_setup.board[0] ?? 0 }, { t: 2, e: "win", side: "P" }] };
  const result = { win: true, score: 5, turns: req.run_step ?? 0 };
  const playerSnapshot = { v: 1, game_v: req.client_build, seed: req.player_setup.seed, actions: [], result };
  return { replay, result, playerSnapshot, ghostHash: ghost.hash };
}
__name(runServerSim, "runServerSim");
__name2(runServerSim, "runServerSim");
var onRequestOptions = /* @__PURE__ */ __name2(async ({ request, env }) => {
  const headers = { ...securityHeaders, ...corsFor(env, request) };
  return new Response(null, { status: 204, headers });
}, "onRequestOptions");
var onRequestPost = /* @__PURE__ */ __name2(async ({ request, env }) => {
  const headers = { "Content-Type": "application/json", ...securityHeaders, ...corsFor(env, request) };
  try {
    ...
    const resp = { ok: true, result: simOut.result, ghost_hash: simOut.ghostHash, replay_id: replayId, replay: simOut.replay };
...
// .wrangler/tmp/bundle-00ZMSs/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-00ZMSs/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.08712236036986098.js.map
